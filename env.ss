...
(define change-env
  (lambda (env sym val)
	...
	(let ([syms (caar env)]
	  ...
	  (let ((pos (find-position sym syms)))
	    (if (number? pos)
		(begin (vector-set! vals pos val) e)
		(cons (car env) (change-env e sym val))))))))
